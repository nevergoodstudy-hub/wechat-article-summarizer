...
from wechat_summarizer.shared.exceptions import ScraperBlockedError, ScraperError
...
def test_scraper_blocks_on_unsafe_dns_resolution(scraper_cls, url: str):
    ...
    with patch(
        "wechat_summarizer.infrastructure.adapters.scrapers.wechat_httpx.resolve_and_validate_host",
        return_value=(False, None),
    ), patch(
        "wechat_summarizer.infrastructure.adapters.scrapers.generic_httpx.resolve_and_validate_host",
        return_value=(False, None),
    ):
        with pytest.raises((ScraperBlockedError, ScraperError)):
