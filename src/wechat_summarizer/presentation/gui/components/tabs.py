...
class TabIndicator:
    ...
    def __init__(
        self, canvas: tk.Canvas, color: str, height: int = 3, animation_duration: int = 200
    ):
        """初始化指示器
        ...
        self._indicator_id = self._canvas.create_rectangle(
            0, 0, 0, 0, fill=color, outline="", tags="indicator"
        )
    ...
    def _update_rect(self):
        """更新矩形位置"""
        self._canvas.coords(
            self._indicator_id, self._x, self._y, self._x + self._width, self._y + self._height
        )
...
class ModernTabs(tk.Frame):
    ...
    def _setup_ui(self):
        ...
        self._tab_bar_canvas = tk.Canvas(
            self._tab_bar_frame, bg=self._colors["tab_bar_bg"], highlightthickness=0, height=40
        )
        ...
        self._indicator = TabIndicator(
            self._tab_bar_canvas, color=self._colors["indicator"], height=3, animation_duration=200
        )
    ...
    def _create_tab_button(self, tab: TabItem):
        ...
        if _CTK_AVAILABLE:
            ...
            def on_label_leave(
                _event: tk.Event[tk.Misc], b: tk.Button = label_btn, tid: str = tab.id
            ) -> None:
                b.configure(
        ...
        # 存储引用
        self._tab_buttons[tab.id] = {"frame": btn_frame, "label": label_btn, "close": close_btn}
    ...
    def _get_tab(self, tab_id: str) -> TabItem | None:
        """获取标签数据"""
    ...
    def _get_tab_index(self, tab_id: str | None) -> int:
        """获取标签索引"""
        if tab_id is None:
            return -1
    ...
    def _update_tab_styles(self):
        ...
        for tab_id, widgets in self._tab_buttons.items():
            ...
            if _CTK_AVAILABLE:
                if is_active:
                    label_btn.configure(
                        text_color=self._colors["text"], fg_color=self._colors["tab_active_bg"]
                    )
                else:
                    label_btn.configure(
                        text_color=self._colors["text_secondary"], fg_color="transparent"
                    )
            else:
                if is_active:
                    ...
                    label_btn.configure(
                        fg=self._colors["text_secondary"], bg=self._colors["tab_bar_bg"]
                    )
    ...
    def _on_drag_start(self, event: tk.Event[tk.Misc], tab_id: str) -> None:
        """开始拖拽"""
