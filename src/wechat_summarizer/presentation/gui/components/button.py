...
class RippleEffect:
    ...
    def __init__(self, canvas: tk.Canvas, color: str = "#FFFFFF", duration: int = 400):
        ...
        self._ripples: list[dict[str, Any]] = []  # 当前活动的水波纹
        self._animation_ids: list[str] = []

    def trigger(self, x: int, y: int, max_radius: int | None = None):
        """触发水波纹动画
...
class ModernButton(ctk.CTkButton if _CTK_AVAILABLE else tk.Button):  # type: ignore[misc]
    """现代按钮组件
    ...
    def _create_ripple_canvas(self):
        ...
        self._ripple_canvas = tk.Canvas(
            self, width=width, height=height, highlightthickness=0, bg=bg_color
        )
    ...
    def set_loading(self, loading: bool):
        ...
        if loading:
            ...
            if _CTK_AVAILABLE:
                self.configure(
                    text=self._text, state="normal" if not self._disabled else "disabled"
                )
            else:
                self.configure(
                    text=self._text, state="normal" if not self._disabled else "disabled"
...
class ButtonGroup:
    ...
    def add_button(self, text: str, command: Callable | None = None, **kwargs) -> ModernButton:
        """添加按钮到组
...
def create_icon_button(
    master, icon: Any, command: Callable | None = None, theme: str = "dark", **kwargs
) -> IconButton:
    """快速创建图标按钮
