...
class Modal:
    ...
    def _create_header(self):
        ...
        if _CTK_AVAILABLE:
            header = ctk.CTkFrame(
                self._modal_frame, fg_color=self._colors["header_bg"], height=50, corner_radius=0
            )
        else:
...
class ConfirmModal(Modal):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            master, title=title, size=ModalSize.SMALL, theme=theme, on_close=on_cancel, **kwargs
        )
    ...
    def _setup_content(self):
        ...
        if _CTK_AVAILABLE:
            confirm_btn = ctk.CTkButton(
                ...
                if self._theme == "dark"
                else ModernColors.LIGHT_ACCENT,
...
class AlertModal(Modal):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            master, title=title, size=ModalSize.SMALL, theme=theme, on_close=on_close, **kwargs
        )
    ...
    def _setup_content(self):
        ...
        if _CTK_AVAILABLE:
            icon_label = ctk.CTkLabel(
                content, text=icon, font=("Arial", 32), text_color=self._get_type_color()
            )
        else:
            icon_label = tk.Label(
                content,
...
# 便捷函数
def show_modal(
    master, title: str = "标题", size: ModalSize = ModalSize.MEDIUM, theme: str = "dark"
) -> Modal:
    """显示模态框
...
def show_alert(
    master, message: str, title: str = "提示", alert_type: str = "info", theme: str = "dark"
) -> AlertModal:
    """显示提示对话框
    ...
    Returns:
        AlertModal实例
    """
    modal = AlertModal(master, title=title, message=message, alert_type=alert_type, theme=theme)
    modal.open()
    return modal
