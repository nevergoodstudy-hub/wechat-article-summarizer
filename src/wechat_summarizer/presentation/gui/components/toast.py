    ...
    def __init__(
        ...
        if _CTK_AVAILABLE:
            ...
            self._frame = tk.Frame(
                master, bg=colors["bg"], highlightthickness=1, highlightbackground=colors["border"]
            )
        ...
        if _CTK_AVAILABLE:
            icon_label = ctk.CTkLabel(
                content_frame, text=icon_text, text_color=colors["icon"], font=("Arial", 16)
            )
        else:
            icon_label = tk.Label(
                content_frame,
...
class ToastManager:
    ...
    def show(
        self, message: str, toast_type: ToastType = ToastType.INFO, duration: int = 3000
    ) -> Toast:
        """显示Toast
