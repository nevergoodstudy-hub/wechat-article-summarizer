...
class ModernInput(ctk.CTkEntry if _CTK_AVAILABLE else tk.Entry):  # type: ignore[misc]
    """现代输入框组件
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if _CTK_AVAILABLE:
            ctk.CTkEntry.pack(
                self, fill="x", padx=0, pady=(8 if self._use_floating_label else 5, 5)
            )
        else:
    ...
    def _create_clear_button(self):
        """创建清除按钮"""
        color = (
            ModernColors.DARK_TEXT_MUTED if self._theme == "dark" else ModernColors.LIGHT_TEXT_MUTED
        )
    ...
    def set_validation(self, state: ValidationState, message: str | None = None):
        """设置验证状态
...
class ModernTextArea(ctk.CTkTextbox if _CTK_AVAILABLE else tk.Text):  # type: ignore[misc]
    """现代多行文本框
...
class PasswordInput(ModernInput):
    ...
    def __init__(self, master, label: str = "密码", theme: str = "dark", **kwargs):
        """初始化密码输入框
        ...
        super().__init__(
            master, label=label, theme=theme, show="*" if not _CTK_AVAILABLE else None, **kwargs
        )
...
# 便捷函数
def create_input(
    master, label: str | None = None, placeholder: str = "", theme: str = "dark", **kwargs
) -> ModernInput:
    """快速创建输入框
...
def create_textarea(
    master, label: str | None = None, height: int = 120, theme: str = "dark", **kwargs
) -> ModernTextArea:
    """快速创建文本框
