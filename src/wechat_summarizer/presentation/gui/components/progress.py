...
class LinearProgress(ctk.CTkProgressBar if _CTK_AVAILABLE else tk.Canvas):  # type: ignore[misc]
    """线性进度条
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if _CTK_AVAILABLE:
            ...
            # Tkinter canvas实现
            super().__init__(
                master, width=width, height=height, bg=bg_color, highlightthickness=0, **kwargs
            )
            ...
            # 进度条
            self._progress_rect = self.create_rectangle(0, 0, 0, height, fill=fg_color, outline="")
    ...
    def update_theme(self, mode: str) -> None:
        ...
        if _CTK_AVAILABLE:
            self.configure(fg_color=bg_color, progress_color=fg_color)
...
class CircularProgress:
    ...
    def __init__(
        ...
        # 颜色
        if theme == "dark":
            bg_color = ModernColors.DARK_BG
            fg_color = ModernColors.DARK_ACCENT
            track_color = ModernColors.DARK_CARD
        else:
            bg_color = ModernColors.LIGHT_BG
            fg_color = ModernColors.LIGHT_ACCENT
            track_color = ModernColors.LIGHT_CARD

        # Canvas
        self._canvas = tk.Canvas(master, width=size, height=size, bg=bg_color, highlightthickness=0)

        # 绘制轨道圆圈
        padding = width
        self._canvas.create_oval(
            padding, padding, size - padding, size - padding, outline=track_color, width=width
        )
...
class StepProgress:
    ...
    def __init__(self, master, steps: list[str], theme: str = "dark"):
        """初始化步骤进度器
        ...
        # 创建步骤指示器
        self._step_widgets: list[tuple[tk.Canvas, tk.Widget]] = []
        self._create_steps()
    ...
    def _create_steps(self):
        ...
        for i, step_name in enumerate(self._steps):
            ...
            if _CTK_AVAILABLE:
                label = ctk.CTkLabel(
                    step_frame, text=step_name, text_color=text_color, font=("Arial", 12)
                )
            else:
...
def create_linear_progress(
    master, width: int = 300, indeterminate: bool = False, theme: str = "dark"
) -> LinearProgress:
    """快速创建线性进度条
...
def create_circular_progress(
    master, size: int = 100, indeterminate: bool = False, theme: str = "dark"
) -> CircularProgress:
    """快速创建圆形进度条
