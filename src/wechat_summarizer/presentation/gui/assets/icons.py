        ...
        for match in cls.COMMAND_PATTERN.finditer(path_data):
            ...
            if cmd_upper == "M":  # 移动
                ...
                for x in args:
                    ...
                    current_x = x
                    commands.append(
                        ("line", [(x * scale + offset_x, current_y * scale + offset_y)])
                    )
                ...
                for y in args:
                    ...
                    current_y = y
                    commands.append(
                        ("line", [(current_x * scale + offset_x, y * scale + offset_y)])
                    )
...
class IconManager:
    ...
    def _create_icon(self, name: str, size: int, color: str) -> Image.Image | None:
        """创建图标 - 使用SVG路径渲染
    ...
    def _render_with_cairosvg(self, path_data: str, size: int, color: str) -> Image.Image | None:
        """使用cairosvg渲染SVG"""
        try:
            # 构建完整SVG
    ...
    def _render_with_pil(self, path_data: str, size: int, color: str) -> Image.Image | None:
        """使用PIL手动渲染SVG路径"""
        # 创建透明背景图像
            ...
            if cmd == "move":
                ...
                if current_pos:
                    curve_points = self._bezier_curve(
                        current_pos, points[0], points[1], points[2], steps=8
                    )
                ...
                # 二次贝塞尔曲线 - 使用线段近似
                if current_pos:
                    curve_points = self._quad_bezier(current_pos, points[0], points[1], steps=6)
    ...
    @staticmethod
    def _draw_polygon(draw: Any, points: list[tuple], color: tuple):
        """绘制填充多边形"""
        if len(points) >= 3:
            # 展平点列表
    ...
    @staticmethod
    def _quad_bezier(
        p0: tuple[float, float], p1: tuple[float, float], p2: tuple[float, float], steps: int = 8
    ) -> list[tuple[float, float]]:
        """计算二次贝塞尔曲线点"""
        points = []
        for i in range(steps + 1):
    ...
    def get_icon_tk(self, name: str, size: IconSize = IconSize.MEDIUM, color: str = "#FFFFFF"):
        """获取Tkinter兼容的图标
...
# 便捷函数
def get_icon(name: str, size: IconSize = IconSize.MEDIUM, color: str = "#FFFFFF"):
    """快速获取图标
...
def get_icon_tk(name: str, size: IconSize = IconSize.MEDIUM, color: str = "#FFFFFF"):
    """快速获取Tkinter图标
