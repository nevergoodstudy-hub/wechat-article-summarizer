...
class GradientManager:
    ...
    def __init__(self):
        self._active_animations: dict[int, dict[str, Any]] = {}
    ...
    def create_linear_gradient(
        self, colors: list[str], steps: int = 10, angle: float = 0.0
    ) -> list[str]:
        """创建线性渐变色列表
    ...
    def create_radial_gradient(
        self, colors: list[str], steps: int = 10, center: tuple[float, float] = (0.5, 0.5)
    ) -> list[str]:
        """创建径向渐变色列表
...
class GradientAnimator:
    ...
    def create_color_flow_animation(
        ...
        def get_frame_color(progress: float) -> str:
            ...
            return manager.interpolate_color(
                colors[segment_index], colors[segment_index + 1], local_progress
            )
    ...
    def get_current_color(self, anim_id: int) -> str | None:
        ...
        color_getter = cast(Callable[[float], str], anim["get_color"])
