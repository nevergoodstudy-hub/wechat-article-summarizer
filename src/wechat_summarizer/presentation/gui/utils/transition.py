...
class PageTransition:
    ...
    def _apply_animation_frame(
        ...
        if trans_type == TransitionType.FADE:
            ...
            new_page.place(
                relx=0.5, rely=0.5, anchor="center", width=max(1, width), height=max(1, height)
            )
    ...
    def _instant_switch(self, new_page: tk.Widget, on_complete: Callable[[], None] | None):
        """即时切换（无动画）"""
...
if __name__ == "__main__":
    ...
    for i, color in enumerate(colors):
        ...
        tk.Label(
            page, text=f"页面 {i + 1}", bg=color, fg="#ffffff", font=("Segoe UI", 24, "bold")
        ).pack(expand=True)
    ...
    router = PageRouter(
        container,
        TransitionConfig(
            type=TransitionType.SLIDE_LEFT, duration=400, easing=EasingFunction.EASE_OUT_CUBIC
        ),
    )
