...
def run_accessibility_demo() -> None:
    ...
    SkipLink(root, text="跳到主要内容", target=main_content)
    ...
    for index, text in enumerate(["首页", "文章", "设置", "帮助"]):
        ...
        focus_manager.register(btn, tab_index=index, group="nav", label=text)
    ...
    _add_labeled_entry(form_frame, "用户名:")
    ...
    _add_labeled_entry(form_frame, "密码:")
