...
class RippleEffect:
    ...
    def __init__(
        self, widget: tk.Widget, color: str = "#ffffff", duration: int = 400, max_radius: int = 100
    ):
        self.widget = widget
        self.color = color
    ...
    def _on_click(self, event):
        ...
        self._canvas = tk.Canvas(
            self.widget, width=widget_width, height=widget_height, highlightthickness=0, bg=""
        )
        ...
        self._ripple_id = self._canvas.create_oval(
            click_x, click_y, click_x, click_y, fill=self.color, outline=""
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._highlight = self.canvas.create_rectangle(
            -50, 0, 0, height, fill=highlight_color, outline=""
        )
...
class CollapseExpand:
    ...
    def toggle(self, on_complete: Callable[[], None] | None = None):
        """切换状态"""
        if self._is_animating:
            return
    ...
    def expand(self, on_complete: Callable[[], None] | None = None):
        """展开"""
    ...
    def collapse(self, on_complete: Callable[[], None] | None = None):
        """收起"""
    ...
    def _animate(self, from_height: int, to_height: int, on_complete: Callable[[], None]):
        """执行动画"""
        self._is_animating = True
        steps = self.duration // 16
...
class MicroInteractions:
    ...
    @classmethod
    def add_ripple(
        cls, widget: tk.Widget, color: str = "#ffffff", duration: int = 400
    ) -> RippleEffect:
        """添加水波纹效果"""
    ...
    @classmethod
    def add_scale(
        cls, widget: tk.Widget, scale_down: float = 0.95, duration: int = 100
    ) -> ScaleEffect:
        """添加缩放效果"""
    ...
    @classmethod
    def add_pulse(
        cls, widget: tk.Widget, color1: str = "#3b82f6", color2: str = "#60a5fa"
    ) -> PulseEffect:
        """添加脉冲效果"""
    ...
    @classmethod
    def add_focus_ring(cls, widget: tk.Widget, color: str = "#3b82f6", width: int = 2) -> FocusRing:
        """添加焦点环"""
...
if __name__ == "__main__":
    ...
    # 标题
    tk.Label(
        root, text="微交互动画演示", bg="#121212", fg="#e5e5e5", font=("Segoe UI", 16, "bold")
    ).pack(pady=20)
    ...
    tk.Label(
        hover_frame, text="悬停查看上浮效果", bg="#2a2a2a", fg="#e5e5e5", font=("Segoe UI", 12)
    ).pack()
    MicroInteractions.add_hover(hover_frame, hover_bg="#3a3a3a", normal_bg="#2a2a2a")

    # Skeleton加载
    tk.Label(root, text="Skeleton 骨架屏:", bg="#121212", fg="#808080", font=("Segoe UI", 10)).pack(
        pady=(20, 5)
    )
    ...
    # Spinner
    tk.Label(
        root, text="Spinner 加载指示器:", bg="#121212", fg="#808080", font=("Segoe UI", 10)
    ).pack(pady=(20, 5))
