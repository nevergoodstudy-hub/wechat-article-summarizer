...
class WeChatLinkDetector:
    ...
    def is_valid_wechat_link(cls, url: str) -> bool:
        ...
        try:
            ...
            return any(parsed.path.startswith(p) for p in valid_paths)
    ...
    def normalize_url(cls, url: str) -> str:
        ...
        try:
            ...
            # 过滤参数
            filtered = {k: v[0] for k, v in params.items() if k in core_params and v}
...
"@
            ...
            if window_title and any(
                kw in window_title.lower() for kw in ["微信", "wechat", "mp.weixin"]
            ):
    ...
    def detect(smart_dedup: bool = True) -> DetectionResult:
        ...
        if clipboard_content:
            links, dup_count, invalid_count = WeChatLinkDetector.extract_links(
                clipboard_content, smart_dedup=smart_dedup
            )
            if links:
