...
class BreakpointManager:
    ...
    def __init__(self, root: tk.Misc, config: BreakpointConfig | None = None):
        ...
        # 使用安全的轮询方式检测窗口大小变化
        # 避免 <Configure> 绑定在 PyInstaller 环境中触发 nametowidget 异常
        self._last_width = 0
        self._last_height = 0
    ...
    def off_breakpoint_change(self, callback: Callable[[Breakpoint, int, int], None]):
        """移除断点变化回调"""
...
if __name__ == "__main__":
    ...
    # 信息标签
    info_label = tk.Label(
        root, text="调整窗口大小查看断点变化", bg="#121212", fg="#e5e5e5", font=("Segoe UI", 14)
    )
