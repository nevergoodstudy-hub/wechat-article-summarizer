...
class WechatArticleFetcher:
    ...
    async def get_article_list(
        ...
        async with self._rate_limiter:
            try:
                response = await self._do_fetch(client, account.fakeid, begin, min(count, 10))
    ...
    def _parse_response(self, response: dict[str, Any]) -> tuple[list[ArticleListItem], int]:
        """解析API响应"""
        items = []
    ...
    async def get_all_articles(
        ...
        while True:
            items, total_count = await self.get_article_list(account, begin=begin, count=page_size)
            ...
            logger.info(f"获取 {account.nickname} 文章进度: {article_list.count}/{total_count}")
    ...
    async def fetch_multiple_accounts(
        ...
        logger.info(f"并发获取完成: 成功 {success_count}/{len(accounts)}")
